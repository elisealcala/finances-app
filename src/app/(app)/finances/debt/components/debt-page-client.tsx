...
import { PayoffScheduleRail } from "./payoff-schedule-rail";
...
export function DebtPageClient() {
  ...
  const [focusedMonthLabel, setFocusedMonthLabel] = useState<string | null>(
    null,
  );
  ...
  // Compute the projection once and share it with the chart + payoff rail so
  // they never disagree.
  const { rows: timelineRows, paymentMarkers } = useMemo(
    () => generateTimeline(visibleDebts),
    [visibleDebts],
  );
  const milestones = useMemo(
    () => getPayoffMilestones(visibleDebts, timelineRows),
    [visibleDebts, timelineRows],
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => router.push("/finances/debt/new")}>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <DebtTimelineChart
            ...
            paymentMarkers={paymentMarkers}
            focusedMonthLabel={focusedMonthLabel}
            onFocusMonth={setFocusedMonthLabel}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <DebtTable
            ...
            onView={(debt) => router.push(`/finances/debt/${debt.id}`)}
